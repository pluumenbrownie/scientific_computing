...
dt = 0.15 * dx**2 / D  # stability condition
...
def update_concentration():
    for i, j in ti.ndrange((1,N), (N+1)):  # exclude boundaries
        c_new[i, j] = c[i, j] + (dt * D / dx**2) * (
            c[i+1, j] + c[i-1, j] + c[i, (j+1) % (N+1)] + c[i, (j-1) % (N+1)] - 4 * c[i, j])
...
for idx, t in enumerate(times):
    im = axes[idx].imshow(c_results[min(int(t / dt) // 100, len(c_results) - 1)], cmap='hot', origin='lower', extent=[0,1,0,1])
    axes[idx].set_title(f't = {t}')
    axes[idx].set_xlabel('x')
    axes[idx].set_ylabel('y')
fig.colorbar(im, ax=axes, orientation='vertical', fraction=0.05)
plt.savefig("local/1.2heatmap.png", dpi=300)
plt.show()
...
ani.save("local/animation_heatmap.gif", writer="pillow", fps=30) # save a gif heatmap
plt.show()
