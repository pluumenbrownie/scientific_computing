...
def calculate_growth_probabilities(
    grid: NDArray[np.int_], candidates: List[Tuple[np.int_, np.int_]], eta: np.float64
) -> NDArray[np.float64]:
    """Calculate the probability of growth for each candidate."""
    concentrations: NDArray[np.float64] = np.array(
        [np.random.rand() for _ in candidates]
    )
...
def simulate_dla(
    grid: NDArray[np.int_], steps: np.int_ = 5000, eta: np.float64 = 1.0
) -> NDArray[np.int_]:
    ...
    for _ in range(steps):
        ...
        probabilities: NDArray[np.float64] = calculate_growth_probabilities(
            grid, candidates, eta
        )
